{
  ...
  let
    ...
    robotApiPkg = pkgs.stdenv.mkDerivation {
      ...
      # Just copy the source; we expect install / build to happen in a container or at runtime.
      buildPhase = ''
        mkdir -p $out
        cp -R . $out/
      '';
